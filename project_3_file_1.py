...
#Number of Documents
db.newyorkosm.find().count()

#Number of Nodes
db.newyorkosm.find({"type":"node"}).count()

#Number of Ways:

db.newyorkosm.find({"type":"way"}).count()

#Number of Unique users
print len(db.newyorkosm.distinct("created.user"))
...
result = db.newyorkosm.find({"address.postcode": {"$regex": "/^[A-Z][a-z]+/"}})

for a in result:
    pprint.pprint(a)

result = db.newyorkosm.find({"address.postcode": {"$regex": "/^\w+$"}})

for a in result:
    pprint.pprint(a)


result = db.newyorkosm.find({"address.housenumber": {"$regex":"/[a-zA-Z]/"}})


for a in result:
    pprint.pprint(a)

result = db.newyorkosm.find({"address.housenumber": {"$regex":"/\s/"}})
